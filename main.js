...
ipcMain.handle('show-dashboard-for-choice', async () => {
    console.log('Showing dashboard for user to choose a session to print.');
    try {
        // If the dashboard window is closed, create it.
        if (!mainWindow || mainWindow.isDestroyed()) {
            createMainWindow();
        }
        // If the window is minimized or hidden, show it.
        mainWindow.show();
        // Bring the window to the front.
        mainWindow.focus();

        // Optional: Show a notification to guide the user.
        new Notification({
            title: 'Choose an Item to Print',
            body: 'Select a session or app from the dashboard, then use its print button.'
        }).show();
...
function stopTracking() {
    ...
    // Process and save session data only if there's something to save
    if (currentSessionActivities.length > 0) {
        // Group activities by apps for this session
        const sessionApps = {};
        currentSessionActivities.forEach(activity => {
        ...
        const completeSessionData = {
            sessionID: sessionID,
            sessionNumber: sessionCounter, // Use the CURRENT session number (e.g., 1)
            startTime: sessionData.startTime.toISOString(),
            endTime: sessionData.endTime.toISOString(),
            duration: duration,
            totalActivities: currentSessionActivities.length,
        ...
        // Save the file using the CURRENT session number.
        const filename = `session_${sessionCounter}_${Date.now()}.json`;
        fs.writeFileSync(path.join(SESSIONS_DIR, filename), JSON.stringify(completeSessionData, null, 2));
        ...
        // --- THE ONLY INCREMENT ---
        // Increment the counter ONCE, AFTER the session is saved.
        sessionCounter++;
        saveSessionCounter(); // Save the new value (e.g., 2) for the next session.
    ...
    // Log the final status. These will now be correct.
    console.log(`✅ Session #${sessionCounter - 1} completed and saved successfully`);
    console.log(`🔄 Ready for Session #${sessionCounter}`);
}
...
ipcMain.handle('print-last-session', async () => {
    try {
        // Find the last session file
        const sessionFiles = fs.readdirSync(SESSIONS_DIR).filter(file => file.endsWith('.json'));
        if (sessionFiles.length === 0) {
            new Notification({ title: 'No Data', body: 'No saved sessions found.' }).show();
            return { success: false };
        }
        sessionFiles.sort((a, b) => {
            const timeA = parseInt(a.split('_').pop().replace('.json', ''), 10);
        ...
        const sessionData = JSON.parse(fs.readFileSync(path.join(SESSIONS_DIR, lastSessionFile)));

        // Ensure the dashboard window is ready
        if (!mainWindow || mainWindow.isDestroyed()) {
            createMainWindow();
            await new Promise(resolve => mainWindow.webContents.once('did-finish-load', resolve));
        }
        ...
        // The rest of the print flow remains the same
        await new Promise(resolve => ipcMain.once('report-ready-for-pdf', resolve));
        mainWindow.webContents.send('trigger-pdf-generation');
...
ipcMain.handle('print-app-list', async () => {
    try {
        // 1. Ensure the dashboard window is open and ready.
        if (!mainWindow || mainWindow.isDestroyed()) {
            createMainWindow();
            await new Promise(resolve => mainWindow.webContents.once('did-finish-load', resolve));
        }

        // 2. Read all session files and aggregate the data.
        const allSessionFiles = fs.readdirSync(SESSIONS_DIR).filter(file => file.endsWith('.json'));
        if (allSessionFiles.length === 0) {
        ...
        allSessionFiles.forEach(file => {
                ...
                if (sessionData && sessionData.apps) {
                    for (const appName in sessionData.apps) {
                        ...
                        if (app.activities && app.activities.length > 1) {
                            app.activities.sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
            ...
            } catch (e) { console.error(`Error processing session file ${file}:`, e); }
        });

        const sortedAppSummary = Object.values(appSummary).sort((a, b) => b.totalActivities - a.totalActivities);

        // 3. Send the final, sorted data to the dashboard to be displayed.
        mainWindow.show();
        mainWindow.webContents.send('generate-app-summary-report', sortedAppSummary);

        // 4. Wait for the dashboard to confirm it has built the HTML.
        await new Promise(resolve => ipcMain.once('report-ready-for-pdf', resolve));

        // 5. Send the final command to the dashboard to trigger the print dialog.
        mainWindow.webContents.send('trigger-pdf-generation');
...
ipcMain.handle('delete-all-sessions', () => {
    try {
        // --- Step 1: Delete all the session files (This part is correct) ---
        const sessionFiles = fs.readdirSync(SESSIONS_DIR);
        for (const file of sessionFiles) {
            fs.unlinkSync(path.join(SESSIONS_DIR, file));
        }
        console.log('✅ All session files have been deleted.');

        // --- Step 2: NEW and CRITICAL - Reset the session counter ---
        sessionCounter = 1; // Reset the counter in memory to 1.
        
        // --- Step 3: NEW and CRITICAL - Save the reset counter back to the file ---
        saveSessionCounter(); // This function will write {"counter": 1} to the JSON file.
        console.log(`🔄 Session counter has been reset to ${sessionCounter} and saved.`);

        // --- Step 4: Tell the dashboard to refresh its (now empty) list (This part is correct) ---
        if (mainWindow && !mainWindow.isDestroyed()) {
            mainWindow.webContents.send('new-session-saved');
        }
        ...
        console.error('Failed to delete sessions and reset counter:', error);
